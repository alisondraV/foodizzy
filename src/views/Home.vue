<template>
  <div>
    ...
    <div class="mt-20 mb-20 mx-8 flex flex-col" v-if="wastedProducts && wastedProducts.length > 0">
      <DonutChart 
        :data="chartData"
        :labels="chartLabels"
      ...
      <div v-for="category in Object.keys(statistics)" :key="category" class="h-30 w-40 flex flex-row">
        <DonutChart
          :data="[statistics[category.toLowerCase()], family.totalProducts[category.toLowerCase()] - statistics[category.toLowerCase()]]"
          :labels="['wasted', 'eaten']"
...
export default class Home extends Vue {
  ...
  get totalProducts() {
    return Object.values(this.family!.totalProducts).reduce((acc, e) => e + acc, 0)
  }
